...
inHowManyYearsChinaWins = 
  let 
    ...
    usa = yearGDP 66000 2
    pairs = zip china usa
  in 1 + (length $ takeWhile less pairs)
...
stat events = map (\(Country name count) -> Country name $ getCountryStat events name) allCountries
  where
    getCountryStat events name =  sum $ map (\(Country name' events') -> if name' /= name then 0 else events') events
