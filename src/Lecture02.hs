...
filter _ Nil = Nil
filter f xs = foldr (\y ys -> if f y then y :. ys else ys) Nil xs
...
reverse Nil = Nil
reverse xs = foldl (\y ys -> ys :. y) Nil xs
...
-}
intercalate :: [a] -> [[a]] -> [a]
intercalate sep ls = P.foldr (\x y -> if null y then x else x ++ sep ++ y) [] ls
